...
const mdComponents = {
  ...
  a: ({ className, children, href, ...props }: MdComponentProps) => (
    <a
      className={cn("font-bold text-neutral-100 hover:text-neutral-200 underline-offset-2 hover:underline break-words", className)}
      href={href}
      target="_blank"
...
const AiMessageBubble: React.FC<AiMessageBubbleProps> = ({
  ...
  return (
    <div className={`relative break-words flex flex-col`}>
      ...
      {(() => {
        console.log("Message debug:", message.id, "has content:", !!message.content, "length:", message.content?.length);
        return null;
      })()}
      {message.content ? (
        <div className="relative">
          ...
          {/* Show typing indicator when still streaming */}
          {isLastMessage && isOverallLoading && (
            <div className="flex items-center gap-2 text-neutral-400 text-sm mt-2 animate-pulse">
              ...
              <span className="text-xs">Generating article...</span>
            </div>
          )}
        </div>
      ) : (
        /* If no content yet, show simple loading */
        <div className="flex items-center gap-2 text-neutral-400 text-sm py-4">
          <Loader2 className="h-4 w-4 animate-spin" />
          <span>Writing article...</span>
        </div>
      )}
      {/* Show copy button if there's substantial content */}
  ...
  return (
    <div className="flex flex-col h-full">
      <ScrollArea className="flex-grow" ref={scrollAreaRef}>
        <div className="p-4 md:p-6 space-y-2 max-w-5xl mx-auto pt-16 overflow-hidden">
          {messages.map((message, index) => {
            const isLast = index === messages.length - 1;
            // Stable key to avoid excessive re-renders
            return (
          ...
          {/* Simple loading indicator - only when needed */}
          {isLoading &&
            (messages.length === 0 ||
